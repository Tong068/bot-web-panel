...
export async function loadConfig() {
  ...
  const publicUrls = Array.isArray(config.publicUrl) ? config.publicUrl : [config.publicUrl]
  if (publicUrls.some(url => typeof url !== 'string')) throw new Error('Invalid publicUrl: expected a URL or an array of URLs')
  for (const value of publicUrls.filter(Boolean)) {
    const url = new URL(value)
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || url.search || url.hash || !['/', base, `${base}/`].includes(url.pathname)) throw new Error('Invalid publicUrl: expected an HTTP(S) origin or /bot-web/ URL')
  }
